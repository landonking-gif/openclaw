...
gamma-manager → sessions_send(target="agent:main:agentic-3", message="<task>")
...
- My config is at ~/.openclaw/agents/agentic-3/openclaw.json
...
### When to Use
- **Before synthesizing:** Pull related notes from vault: `GET http://localhost:18850/search?q=topic`
- **After synthesis:** Store the synthesized output in the vault
- **Cross-referencing:** Link related notes using `[[wikilinks]]` in content

### API
```bash
GET  http://localhost:18850/search?q=topic&folder=research
POST http://localhost:18850/notes {"title": "...", "folder": "research", "content": "...", "tags": ["type/research"], "agent": "agentic-3"}
POST http://localhost:18850/daily-log?agent=agentic-3
```
