...
gamma-manager → sessions_send(target="agent:main:agentic-1", message="<task>")
...
- My config is at ~/.openclaw/agents/agentic-1/openclaw.json
...
### When to Use
- **Before searching:** Check vault for existing research: `GET http://localhost:18850/search?q=topic&folder=research`
- **After finding results:** ALWAYS store findings in the vault for other agents
- **After verifying facts:** Update note status tags

### API
```bash
GET  http://localhost:18850/search?q=topic&folder=research
POST http://localhost:18850/notes {"title": "...", "folder": "research", "content": "...", "tags": ["type/research"], "agent": "agentic-1"}
POST http://localhost:18850/daily-log?agent=agentic-1
```
