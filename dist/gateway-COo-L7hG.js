import { C as textToSpeech } from "./speech-runtime-Bqj7xL3f.js";
import { A as PLUGIN_USER_AGENT, B as sendC2CVideoMessage, C as audioFileToSilkBase64, D as isVoiceAttachment, E as isGlobalTTSAvailable, F as onMessageSent, G as sendGroupImageMessage, H as sendChannelMessage, I as sendC2CFileMessage, J as sendGroupVoiceMessage, K as sendGroupMessage, L as sendC2CImageMessage, M as getAccessToken, N as getGatewayUrl, O as resolveTTSConfig, P as initApiConfig, Q as stopBackgroundTokenRefresh, R as sendC2CInputNotify, S as readFileAsync, T as formatDuration, U as sendDmMessage, V as sendC2CVoiceMessage, W as sendGroupFileMessage, Z as startBackgroundTokenRefresh, _ as normalizeMediaTags, at as resolveQQBotLocalMediaPath, b as fileExistsAsync, c as sendPhoto, ct as debugError, d as sendVideoMsg, et as getQQBotDataDir, f as sendVoice, g as parseQQBotPayload, h as isMediaPayload, it as normalizePath, j as clearTokenCache, k as textToSilk, lt as debugLog, m as isCronReminderPayload, nt as isLocalPath, o as sendDocument, ot as runDiagnostics, p as encodePayloadForCron, q as sendGroupVideoMessage, s as sendMedia, st as sanitizeFileName, tt as getQQBotMediaDir, v as checkFileSize, w as convertSilkToWav, x as formatFileSize, y as downloadFile, z as sendC2CMessage } from "./outbound-UcsqJEAJ.js";
import { t as getQQBotRuntime } from "./runtime-DEaVpQaw.js";
import { a as chunkText, i as TEXT_CHUNK_LIMIT, n as matchSlashCommand, r as qqbotPlugin } from "./slash-commands-C0nLfWw7.js";
import { a as recordKnownUser, n as flushKnownUsers } from "./known-users-BCF0FsV1.js";
...
async function handleVideoPayload(ctx, payload) {
	...
	try {
		...
		if (!videoPath?.trim()) log?.error(`[qqbot:${account.accountId}] Video missing path`);
		else {
			...
			await sendWithTokenRetry(account.appId, account.clientSecret, async (token) => {
				if (isHttpUrl) {
					...
					if (!await fileExistsAsync(videoPath)) throw new Error(`Video file does not exist: ${videoPath}`);
					const vPaySzCheck = checkFileSize(videoPath);
					if (!vPaySzCheck.ok) throw new Error(vPaySzCheck.error);
...
async function handleFilePayload(ctx, payload) {
	...
	try {
		...
		if (!filePath?.trim()) log?.error(`[qqbot:${account.accountId}] File missing path`);
		else {
			...
			await sendWithTokenRetry(account.appId, account.clientSecret, async (token) => {
				if (isHttpUrl) {
					...
					if (!await fileExistsAsync(filePath)) throw new Error(`File does not exist: ${filePath}`);
					const fPaySzCheck = checkFileSize(filePath);
					if (!fPaySzCheck.ok) throw new Error(fPaySzCheck.error);
					const fileBase64 = (await readFileAsync(filePath)).toString("base64");
