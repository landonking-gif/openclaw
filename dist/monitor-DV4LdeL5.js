import { c as normalizeAgentId, u as resolveAgentIdFromSessionKey } from "./session-key-4QR94Oth.js";
...
import { h as sendMediaWithLeadingCaption, m as resolveTextChunksWithFallback, p as resolveSendableOutboundReplyParts } from "./reply-payload-DFX4yBqp.js";
...
import { a as isRequestBodyLimitError, c as requestBodyErrorToText, i as installRequestBodyLimitGuard, s as readRequestBodyWithLimit } from "./http-body-BYXg46U-.js";
...
import { a as createFeishuClient, c as listEnabledFeishuAccounts, d as resolveFeishuAccount, f as resolveFeishuRuntimeAccount, i as createEventDispatcher, n as raceWithTimeoutAndAbort, o as createFeishuWSClient, r as waitForAbortableDelay, t as probeFeishu } from "./probe-CyT3gzP1.js";
import { E as isMentionForwardRequest, M as getFeishuRuntime, O as replyComment, _ as normalizeFeishuExternalKey, b as extractMentionTargets, c as getMessageFeishu, d as sendCardFeishu, g as parsePostContent, i as sendMediaFeishu, l as listFeishuThreadMessages, m as sendStructuredCardFeishu, p as sendMessageFeishu, t as downloadMessageResourceFeishu, u as resolveFeishuCardTemplate, v as buildMentionedCardContent } from "./media-BnfgFElW.js";
import "./runtime-api-DJM9ExNn.js";
import { r as resolveReceiveIdType } from "./targets-BeNrIVh4.js";
import { a as resolveFeishuReplyPolicy, c as createFeishuCardInteractionEnvelope, l as decodeFeishuCardAction, n as resolveFeishuAllowlistMatch, r as resolveFeishuGroupConfig, s as buildFeishuCardActionTextFallback, t as isFeishuGroupAllowed } from "./policy-TvbJKTgo.js";
import { t as buildFeishuConversationId } from "./conversation-id-iPkVbiry.js";
import { n as createFeishuThreadBindingManager } from "./thread-bindings-D1gmTdTW.js";
import { n as normalizeCommentFileType, t as buildFeishuCommentTarget } from "./comment-target-BtR-mdvR.js";
...
function createFeishuCommentReplyDispatcher(params) {
	...
	const { dispatcher, replyOptions, markDispatchIdle } = core.channel.reply.createReplyDispatcherWithTyping({
		...
		deliver: async (payload, info) => {
			...
			for (const chunk of chunks) await replyComment(client, {
...
async function fetchDriveCommentReplies(params) {
	...
	for (let page = 0; page < FEISHU_COMMENT_REPLY_PAGE_LIMIT; page += 1) {
		...
		if (response?.code !== 0) {
			if (response) params.logger?.(`feishu[${params.accountId}]: failed to fetch comment replies for ${params.commentId}: ${response.msg ?? "unknown error"}`);
			break;
		}
...
async function fetchDriveCommentContext(params) {
	...
	const commentCard = commentResponse?.code === 0 ? (commentResponse.data?.items ?? []).find((item) => item.comment_id?.trim() === params.commentId) ?? commentResponse.data?.items?.[0] : void 0;
	...
	if (!embeddedTargetReply || replies.length === 0) {
		...
		if (fetchedReplies.length > 0) replies = fetchedReplies;
	}
	const rootReply = replies[0] ?? embeddedReplies[0];
	const fetchedMatchedReply = params.replyId ? replies.find((reply) => reply.reply_id?.trim() === params.replyId?.trim()) : void 0;
...
async function resolveDriveCommentEventCore(params) {
	const { cfg, accountId, event, botOpenId, createClient = (account) => createFeishuClient(account), verificationTimeoutMs = FEISHU_COMMENT_VERIFY_TIMEOUT_MS, logger } = params;
...
function isFeishuWebhookSignatureValid(params) {
	...
	return timingSafeEqualString(crypto.createHash("sha256").update(timestamp + nonce + encryptKey + params.rawBody).digest("hex"), signature);
}
