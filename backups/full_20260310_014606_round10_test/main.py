    ...
    try:
        if action == "screenshot_ocr":
            ...
            if _ensure_ocr():
                ...
                tess_check = subprocess.run(["which", "tesseract"], capture_output=True, text=True)
...
def _git_ops(action: str, **kwargs) -> dict:
    ...
    try:
        if action in ("status", "log", "diff", "branch", "stash", "remote", "tag"):
            ...
            if not url:
                return {"error": "url required"}
            cmd = ["git", "clone", "--depth", str(kwargs.get("depth", 0) or 0)]
            if kwargs.get("depth"):
...
tell application "System Events"
    tell process "{app}"
        try
            click {role} "{element_name}" of window 1
            return "clicked"
        on error errMsg
            return "error: " & errMsg
        end try
...
tell application "System Events"
    tell process "{app}"
        try
            return value of {role} "{element_name}" of window 1
        on error errMsg
            return "error: " & errMsg
        end try
    end tell
...
tell application "System Events"
    tell process "{app}"
        try
            set value of {role} "{element_name}" of window 1 to "{escaped}"
            return "set"
        on error errMsg
            return "error: " & errMsg
        end try
