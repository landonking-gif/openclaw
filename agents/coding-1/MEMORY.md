...
beta-manager → sessions_send(target="agent:main:coding-1", message="<task>")
...
- My config is at ~/.openclaw/agents/coding-1/openclaw.json
...
The army has a shared **Obsidian Knowledge Vault** via `http://localhost:18850`.
...
### API
```bash
GET  http://localhost:18850/search?q=python+fastapi
POST http://localhost:18850/notes {"title": "...", "folder": "code-patterns", "content": "...", "tags": ["type/code", "language/python"], "agent": "coding-1"}
POST http://localhost:18850/daily-log?agent=coding-1
```
