...
import { f as readConfigFileSnapshot, x as writeConfigFile } from "./io-D4TfzS5d.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-hQkmBJiq.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-D0HJIN-V.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DArjrEvl.js");
		const { logConfigUpdated } = await import("./logging-BZTM0IIq.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-aiSG_PU7.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-D1MpjQ7I.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-DMdVDwmR.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-DjyZXRC0.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Dv4Iwckt.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CgTgCsg-.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DtHCVsLp.js");
		const { setupChannels } = await import("./onboard-channels-DrhA-ZLS.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-CrYz4O5o.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CmmNiFCk.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-sx4CQtDv.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BtKLN1vr.js");
