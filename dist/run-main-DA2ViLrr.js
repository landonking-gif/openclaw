...
import { t as assertSupportedRuntime } from "./runtime-guard-tsl9GPp6.js";
import "./logging-D3xhgf7Q.js";
import { o as hasMemoryRuntime } from "./memory-state-DuJSytE1.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-QqAVtDUx.js");
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		if (timeoutMs === null) return false;
		if (json) {
			const { statusJsonCommand } = await import("./status-json-C_dc8UsL.js");
		...
		const { statusCommand } = await import("./status-BLfLiWId.js");
		await statusCommand({
			json,
			deep,
			all,
...
const routeGatewayStatus = {
	...
	run: async (argv) => {
		...
		const { runDaemonStatus } = await import("./status-CgSJQbBv.js");
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-DsggCosv.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-XhLt6AXA.js");
...
const routes = [
	...
	routeAgentsList,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-6VeJ29ev.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-6VeJ29ev.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-BN1phYg3.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-BN1phYg3.js");
...
async function prepareRoutedCommand(params) {
	...
	if (!suppressDoctorStdout && process.stdout.isTTY) {
		const [{ emitCliBanner }, { VERSION }] = await Promise.all([import("./banner-typKBdc4.js"), import("./version-Dw-U-Mrc.js")]);
		emitCliBanner(VERSION, { argv: params.argv });
	}
	if (!skipConfigGuard) {
		const { ensureConfigReady } = await import("./config-guard-Baf07829.js");
	...
	if (typeof params.loadPlugins === "function" ? params.loadPlugins(params.argv) : params.loadPlugins) {
		const { ensurePluginRegistryLoaded } = await import("./plugin-registry-mfDBWE-M.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-BQiHTupx.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-s7t6o35p.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputRootHelp } = await import("./root-help-Bdcla7Uo.js");
			await outputRootHelp();
			return;
		}
		if (await tryRouteCli(normalizedArgv)) return;
		enableConsoleCapture();
		const { buildProgram } = await import("./program-Bk3KAelG.js");
		...
		if (primary) {
			const { getProgramContext } = await import("./program-context-NnPDpPt7.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BuxpZU_9.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-B2JHlUfU.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { registerPluginCliCommands } = await import("./cli-ZcN2v_PT.js");
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-B2JHlUfU.js");
