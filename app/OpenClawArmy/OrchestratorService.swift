...
class OrchestratorService: ObservableObject {
    ...
    @Published var memoryServiceHealth: ServiceHealth?
    @Published var knowledgeBridgeHealth: ServiceHealth?
    ...
    func fetchServiceDiagnostic() {
        struct DiagnosticService: Codable {
            let status: String
            let port: Int?
            let error: String?
            let http_status: Int?
        ...
        struct DiagnosticResponse: Codable {
            let services: [String: DiagnosticService]?
        }

        get("/diagnostic") { [weak self] (result: Result<DiagnosticResponse, Error>) in
            DispatchQueue.main.async {
                guard let self = self else { return }
                switch result {
                case .success(let diag):
                    if let mem = diag.services?["memory-service"] {
                        self.memoryServiceHealth = ServiceHealth(
                            status: mem.status,
                    ...
                    if let kg = diag.services?["knowledge-bridge"] {
                        self.knowledgeBridgeHealth = ServiceHealth(
                            status: kg.status,
                ...
                case .failure:
                    self.memoryServiceHealth = ServiceHealth(status: "unknown", service: "memory-service", dependencies: nil, vault: nil, vault_exists: nil)
                    self.knowledgeBridgeHealth = ServiceHealth(status: "unknown", service: "knowledge-bridge", dependencies: nil, vault: nil, vault_exists: nil)
                }
            }
        }
    }
