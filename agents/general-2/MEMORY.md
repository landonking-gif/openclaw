...
alpha-manager → sessions_send(target="agent:main:general-2", message="<task>")
...
- My config is at ~/.openclaw/agents/general-2/openclaw.json
...
### When to Use
- **Before summarizing:** Check vault for existing summaries on the topic
- **After summarizing:** Store the summary in the vault
- **For context:** Pull related notes to improve summary quality

### API
```bash
GET  http://localhost:18850/search?q=topic
POST http://localhost:18850/notes {"title": "...", "folder": "research", "content": "...", "tags": ["type/research"], "agent": "general-2"}
POST http://localhost:18850/daily-log?agent=general-2
```
