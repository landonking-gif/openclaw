...
- **Agent:** general-1
- **Port:** 18811
- **Specialty:** Writing/Content Creation
...
alpha-manager → sessions_send(target="agent:main:general-1", message="<task>")
...
- My config is at ~/.openclaw/agents/general-1/openclaw.json
...
### When to Use
- **Before writing:** Check vault for style guides and existing content
- **After producing content:** Store polished outputs for reuse
- **After learning user preferences:** Document them in vault

### API
```bash
GET  http://localhost:18850/search?q=topic
POST http://localhost:18850/notes {"title": "...", "folder": "research", "content": "...", "tags": ["type/research"], "agent": "general-1"}
POST http://localhost:18850/daily-log?agent=general-1
```
