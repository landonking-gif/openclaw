...
import { a as resolveSecretRefValue } from "./resolve-BuNxIOCX.js";
import { _ as GATEWAY_CLIENT_NAMES, g as GATEWAY_CLIENT_MODES } from "./message-channel-ChfElmW5.js";
import { Ct as validateSecretsResolveResult } from "./method-scopes-ByMTuU41.js";
import { r as callGateway } from "./call-WcuBXDTZ.js";
import { d as getPath, p as setPathExistingStrict, r as discoverConfigSecretTargetsByIds } from "./target-registry-X-cDY-3x.js";
import { d as assertExpectedResolvedSecretValue, l as analyzeCommandSecretAssignmentsFromSnapshot, n as collectConfigAssignments, o as createResolverContext, t as resolveRuntimeWebTools } from "./runtime-web-tools-OaSjm00V.js";
import { s as resolveBundledWebSearchPluginId } from "./web-search-providers.shared-U-IOIzYo.js";
...
function pluginIdFromRuntimeWebPath(path) {
	return /^plugins\.entries\.([^.]+)\.config\.webSearch\.apiKey$/.exec(path)?.[1];
}
...
function classifyRuntimeWebTargetPathState(params) {
	...
	if (params.path === "tools.web.x_search.apiKey") return params.config.tools?.web?.x_search?.enabled !== false ? "active" : "inactive";
...
function describeInactiveRuntimeWebTargetPath(params) {
	if (params.path === "tools.web.fetch.firecrawl.apiKey") {
		...
		if (fetch?.firecrawl?.enabled === false) return "tools.web.fetch.firecrawl.enabled is false.";
		return;
	}
	if (params.path === "tools.web.x_search.apiKey") return params.config.tools?.web?.x_search?.enabled === false ? "tools.web.x_search is disabled." : void 0;
...
function isDirectRuntimeWebTargetPath(path) {
	return path === "tools.web.fetch.firecrawl.apiKey" || path === "tools.web.x_search.apiKey" || /^tools\.web\.search\.[^.]+\.apiKey$/.test(path);
}
