...
import { f as readConfigFileSnapshot, x as writeConfigFile } from "./io-CHHRUM9X.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-2eV11F8w.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-0eWgQt48.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-s0dIJs2L.js");
		const { logConfigUpdated } = await import("./logging-CmDFUvIx.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-Buo76Du2.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-C83D1sMG.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-Z3yDwJBK.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-BSHCrgdB.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-C6XeUSC6.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-B2j8Ku4g.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-D_J_TZC2.js");
		const { setupChannels } = await import("./onboard-channels-Bmr6j4rY.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-k309dcfa.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-5gjGgpzx.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-CT7bzQDG.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-t-7hc-6o.js");
