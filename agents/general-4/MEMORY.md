...
alpha-manager → sessions_send(target="agent:main:general-4", message="<task>")
...
- My config is at ~/.openclaw/agents/general-4/openclaw.json
...
### When to Use
- **Before automating:** Check vault for existing automation scripts
- **After building automations:** Store them as code patterns
- **After Mac integration tasks:** Document capabilities and shortcuts

### API
```bash
GET  http://localhost:18850/search?q=automation+macos
POST http://localhost:18850/notes {"title": "...", "folder": "code-patterns", "content": "...", "tags": ["type/code", "domain/automation"], "agent": "general-4"}
POST http://localhost:18850/daily-log?agent=general-4
```
