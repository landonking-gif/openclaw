...
beta-manager → sessions_send(target="agent:main:coding-3", message="<task>")
...
- My config is at ~/.openclaw/agents/coding-3/openclaw.json
...
### API
```bash
GET  http://localhost:18850/search?q=bash+shell
POST http://localhost:18850/notes {"title": "...", "folder": "code-patterns", "content": "...", "tags": ["type/code", "language/bash"], "agent": "coding-3"}
POST http://localhost:18850/daily-log?agent=coding-3
```
