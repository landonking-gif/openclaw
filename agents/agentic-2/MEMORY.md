...
gamma-manager → sessions_send(target="agent:main:agentic-2", message="<task>")
...
- My config is at ~/.openclaw/agents/agentic-2/openclaw.json
...
### When to Use
- **Before analyzing:** Check vault for existing analysis: `GET http://localhost:18850/search?q=topic`
- **After analysis:** Store findings in the vault
- **After document review:** Create structured notes

### API
```bash
GET  http://localhost:18850/search?q=topic&folder=research
POST http://localhost:18850/notes {"title": "...", "folder": "research", "content": "...", "tags": ["type/research"], "agent": "agentic-2"}
POST http://localhost:18850/daily-log?agent=agentic-2
```
