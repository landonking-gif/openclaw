...
export default function register(api: OpenClawPluginApi) {
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        category:   { type: "string", enum: ["decision", "finding", "preference", "process", "entity", "other"], description: "Category of memory" },
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        category: { type: "string", enum: ["decision", "finding", "preference", "process", "entity", "other"] },
    ...
    execute: async (args: any) => {
      ...
      return result.results
        .map((r: any, i: number) => `${i + 1}. [${r.category}] (score: ${r.score}) ${r.content}`)
        .join("\n");
    },
  });
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      required: ["success"],
      properties: {
        story_id:       { type: "string", description: "Identifier for the task" },
        story_title:    { type: "string", description: "Title of the task" },
        attempt_number: { type: "number", description: "Which attempt (default 1)" },
        success:        { type: "boolean", description: "Did it succeed?" },
        error:          { type: "string", description: "Error message if failed" },
        files_modified: { type: "array", items: { type: "string" }, description: "Files modified" },
      },
    },
    execute: async (args: any) => {
      const result = await httpPost("/memory/diary", {
        agent_name: agentName,
      ...
      return `Diary entry logged: ${result.entry_id} (${args.success ? "success" : "failure"})`;
    },
  });
  ...
  api.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        all_attempts:   { type: "array", items: { type: "object" }, description: "Array of attempt records" },
  ...
  api.registerTool({
    ...
    execute: async (args: any) => {
      const [tier1, tier2] = await Promise.all([
        httpGet(`/memory/tier1/${agentName}/context?session_id=${args.session_id || "default"}`),
        httpGet(`/memory/tier2/${agentName}/context?limit=3`),
      ]);
      const parts: string[] = [];
      if (tier1.context !== "No previous context available.") {
        parts.push("## Recent Messages\n" + tier1.context);
      }
      if (tier2.context !== "No session summaries available.") {
        parts.push(tier2.context);
      }
      return parts.length > 0 ? parts.join("\n\n") : "No memory context available.";
    },
  });

  // ── Hook: auto-commit significant responses to Tier 1 ───────────────────
  api.on("after_tool_call", async (event: any) => {
    // Store in Tier 1 (recent context)
    try {
      if (event.result && typeof event.result === "string" && event.result.length > AUTO_COMMIT_THRESHOLD) {
        await httpPost("/memory/tier1/add", {
          agent_name: agentName,
          role: "assistant",
          content: event.result.substring(0, 1000),
